    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                interactiveMode = GUILayout.Toggle(interactiveMode, interactiveMode ? "ON" : "OFF");
                if (GUI.changed) GameManager.Instance.interactiveMode = interactiveMode;
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Master Volume:", GUILayout.Width(150));
                    float masterVolume = SoundManager.Instance.masterVolume;
                    masterVolume = GUILayout.HorizontalSlider(masterVolume, -80.0f, 20.0f, GUILayout.Height(20), GUILayout.Width(50));
                    GUILayout.Label($"{masterVolume:F1} dB", GUILayout.Width(50));
                    if (GUI.changed) SoundManager.Instance.masterVolume = masterVolume;
                }
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Sound Muted:", GUILayout.Width(150));
                    bool isMuted = SoundManager.Instance.masterMuted;
                    isMuted = GUILayout.Toggle(isMuted, isMuted ? "MUTED" : "UNMUTED");
                    if (GUI.changed) SoundManager.Instance.masterMuted = isMuted;
                }
                GUILayout.EndHorizontal();
